...
client.on("message", message => {
 ...
 if (message.content === "^^help") {
  ...
  const embed = new Discord.RichEmbed()
      ...
      .addField("『المصمم』       LiZer_PRo", true)
...
client.on('ready', () => {
   client.user.setGame(" ^^help | شكشوكة المتناكة ");
}); 
...
if (message.content.startsWith('bc')){
 if(!message.author.id === '532613290795335700') return;
...
client.on('guildMemberAdd', member=> {
    member.addRole(member.guild.roles.find("name","city owns"));
    });
